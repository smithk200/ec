...
static const u16 sBadgeFlags[9] = {
...
const u16 sLevelCaps[NUM_SOFT_CAPS] = { 14, 20, 26, 37, 43, 49, 53, 63, 75 };
const double sLevelCapReduction[7] = { .4, .25, .2, .15, .1, .05, .03 };
...
void SetMoveEffect(bool32 primary, u32 certain)
{
    ...
    if (gBattleScripting.moveEffect <= PRIMARY_STATUS_MOVE_EFFECT) // status change
    {
        ...
        if (statusChanged == TRUE)
        {
            ...
            #else
                gBattleMons[gEffectBattler].status1 |= ((Random() % 3) + 3);
            #endif
            else
...
static void Cmd_getexp(void)
{
    ...
    case 1: // calculate experience points to redistribute
        {
            ...
            for (viaSentIn = 0, i = 0; i < PARTY_SIZE; i++)
            {
                ...
                if (holdEffect == HOLD_EFFECT_EXP_SHARE || FlagGet(FLAG_EXP_SHARE))
                    viaExpShare++;
            }
    ...
    case 2: // set exp value to the poke in expgetter_id and print message
        if (gBattleControllerExecFlags == 0)
        {
            ...
            if ((holdEffect != HOLD_EFFECT_EXP_SHARE && !(gBattleStruct->sentInPokes & 1) && !FlagGet(FLAG_EXP_SHARE))
            ...
            else
            {
                ...
                #else
                    if ((holdEffect == HOLD_EFFECT_EXP_SHARE || FlagGet(FLAG_EXP_SHARE)) && gBattleMoveDamage == 0)
