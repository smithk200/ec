...
static bool32 HasBadOdds2(void)
{
    //Variable initialization
    u32 battler, emitresult;
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        //aiMoveEffect = aiMove.effect; //gBattleMons[gActiveBattler].moves[i]
        if (aiMove != MOVE_NONE)
        {
            ...
            || aiMove == MOVE_EXPLOSION || aiMove == MOVE_SELF_DESTRUCT || aiMove == MOVE_HOLY_DUTY 
            || aiMove == MOVE_SLEEP_POWDER || aiMove == MOVE_YAWN || aiMove == MOVE_LOVELY_KISS || aiMove == MOVE_GRASS_WHISTLE || aiMove == MOVE_HYPNOSIS  || MOVE_SPORE
    ...
    //Calculates the type advantage
    typeDmg *= UQ_4_12_TO_INT(GetTypeModifier(atkType1, defType1));
	...
	if (typeDmg>=UQ_4_12(2.0)) //If the player has a 2x type advantage or greater...
	{
			...
			&& (gBattleMons[gActiveBattler].hp >= gBattleMons[gActiveBattler].maxHP/2)) //If the computer doesn't have a super effective move AND they have >1/2 their HP...
		{
			for (i = 0; i < MAX_MON_MOVES; i++) //Then check their moves to see if they have a status move. If you have a status move, you probably want to use it even if you don't have the advantage.
			{
				...
				|| move == MOVE_EXPLOSION || move == MOVE_SELF_DESTRUCT || move == MOVE_HOLY_DUTY 
				|| move == MOVE_SLEEP_POWDER || move == MOVE_YAWN || move == MOVE_LOVELY_KISS || move == MOVE_GRASS_WHISTLE || move == MOVE_HYPNOSIS  || MOVE_SPORE
...
static bool8 AiCanSuicide(void) //unused and leftover
{
    u16 move;
	s32 i, j;
    if (AI_THINKING_STRUCT->aiFlags & AI_FLAG_WILL_SUICIDE) //SHOULD check to see if suicide flag is triggered.
    {
        for (i = 0; i < MAX_MON_MOVES; i++)
                {
                    move = gBattleMons[gActiveBattler].moves[i];
                    if ((move == MOVE_EXPLOSION || move == MOVE_SELF_DESTRUCT || move == MOVE_HOLY_DUTY
                    || move == MOVE_FINAL_GAMBIT || move == MOVE_MEMENTO)) //if Pokemon has a suicide move, the AI will report "true"
                    {
...
static bool32 AreAttackingStatsLowered(void)
{
    s8 attackingStage = gBattleMons[gActiveBattler].statStages[STAT_ATK];
    s8 spAttackingStage = gBattleMons[gActiveBattler].statStages[STAT_SPATK];
    ...
    if (gBattleMons[gActiveBattler].attack > gBattleMons[gActiveBattler].spAttack)
    {
        ...
        {
            if (AI_DATA->mostSuitableMonId[gActiveBattler] != PARTY_SIZE && (Random() & 1))
            {
